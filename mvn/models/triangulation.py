...
class VolumetricTriangulationNet_jmz(nn.Module):
    def __init__(self, config, device='cuda:0'):
        super().__init__()
        self.cubeSize = [1200, 1400, 2000]
        self.cubeCenter = np.array([0, 0, 1000], dtype=np.float)
        self.coord_volume = self._makeCube()
        self.num_joints = config.model.backbone.num_joints
        self.volume_aggregation_method = config.model.volume_aggregation_method
    ...
    def _makeCube(self):
        ...
        # build cuboid
        sides = np.array(self.cubeSize)
        position = self.cubeCenter - sides / 2
        cuboid = volumetric.Cuboid3D(position, sides)
    ...
    def forward(self, images, proj_matricies, batch):
        ...
        for batch_i in range(batch_size):
            coord_volumes[batch_i] = self.coord_volume.clone().to(device)
            cuboids.append(self.cuboid)
